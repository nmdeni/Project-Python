...
def edit_user(id):
    """
        сюда передается от пользователя id юзера которого надо редактировать
        возвращается словарь работа в цикле юзер указыввет какой параметр хочеь редактировать
        далее редачет и все пишется в бд
    """
    # ЗДЕСЬ ЗАПИСЬ В БД
    return id
...
while start_prog:
    ...
    if user_com.lower() == "q":
        ...
        print(f"{edit_user(input("Кого хотите редактировать => "))} был отредактирован!\n Нажмиете enter!")
        input()